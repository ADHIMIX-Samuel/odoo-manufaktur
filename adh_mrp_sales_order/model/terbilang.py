...
class terbilang(models.Model):
	_name = 'wit.terbilang'

	def terbilang(self, amount, cur, lang='id'):
		if lang=='id':
			number = '%.2f' % amount
			units_name = ' ' + cur_name(cur) + ' '
			lis = str(number).split('.')
			start_word = english_number(int(lis[0]), lang)
			end_word = english_number(int(lis[1]), lang)
			...
			hasil = final_result[:1].upper()+final_result[1:]
		else:
			hasil = amount_to_text_en.amount_to_text(amount, 'en', cur)
