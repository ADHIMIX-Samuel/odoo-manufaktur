...
class report_kesiapan_pondasi(models.Model):
	_name = "report.adh_manufaktur.report_kesiapan_pondasi"

	@api.model
	# def get_kesiapan_pondasi_details(self,tanggal=False):
	def get_kesiapan_pondasi_details(self, date_start =False,date_stop=False):
		...
		produk_silo_coldbin_ids = self.env['adhimix.mrp.instalasi.silo']
		for pondasi_id in kesiapan_pondasi_ids :
			produk_silo_coldbin_ids += pondasi_id.produk_silo_coldbin_ids
	...
	def render_html(self, docids, data=None):
		...
		data.update(self.get_kesiapan_pondasi_details(data['date_start'],data['date_stop']))
		return self.env['report'].render('adh_manufaktur.report_kesiapan_pondasi', data)
